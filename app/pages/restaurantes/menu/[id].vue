<template>
  <div class="mx-auto max-w-4xl px-4 py-6">
    ...
    <div v-else-if="error" class="mt-4 text-terra-500">No se pudo cargar el menú.</div>
    ...
    <div v-else>
      ...
      <div v-else class="mt-6 grid gap-4 sm:grid-cols-2">
        <Card v-for="item in menuItems" :key="item.id" :img="genericImage">
          <template #header>
            <h3 class="text-base font-semibold text-brand-900">{{ item.name }}</h3>
            <p class="text-sm text-brand-700">Q {{ Number(item.price).toFixed(2) }}</p>
          </template>
...
const genericImage = "https://plus.unsplash.com/premium_photo-1673972355945-afb546ffe3ce?q=80&w=2072&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"

const restaurantService = useRestaurantService()
const menuApi = getMenuItems()

// Cargar restaurante (para nombre/cabecera)
const { data: restaurantData, pending: pendingRestaurant, error: errorRestaurant } = await useAsyncData(
  () => `restaurant:${id}`,
...
const { data: itemsData, pending: pendingItems, error: errorItems } = await useAsyncData(
