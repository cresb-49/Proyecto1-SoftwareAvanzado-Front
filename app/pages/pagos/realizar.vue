<template>
  <div class="mx-auto max-w-xl px-4 py-6 space-y-6">
    <!-- Título -->
    <header>
      <h1 class="text-2xl font-semibold text-brand-800">Realizar pagos por semana</h1>
      <p class="text-sm text-brand-700">Selecciona un <strong>lunes</strong> para marcar como pagados todos los pagos de esa semana.</p>
    </header>

    <!-- Formulario -->
    <Card variant="elevated" title="Pagar todos por semana" subtitle="La fecha debe caer en lunes">
      <form class="grid gap-4" @submit.prevent="onSubmit">
        ...
        <div class="flex items-center justify-end gap-2">
          ...
          <Button type="submit" variant="primary" :loading="loading">Realizar pagos</Button>
        </div>
      </form>
    </Card>

    <!-- Resultado -->
    <Card v-if="paidCount !== null" variant="soft">
      <template #title>Resultado</template>
      <p class="text-brand-800">Se marcaron como pagados <strong>{{ paidCount }}</strong> pago(s) correspondientes a la semana que inicia el <strong>{{ weekStart }}</strong>.</p>
      <p v-if="paidCount === 0" class="mt-1 text-sm text-brand-700">No se marcó ningún pago. Verifica que existan pagos pendientes para esa semana.</p>
    </Card>
  </div>
</template>
...
async function onSubmit() {
  paidCount.value = null
  if (!validate()) return
  try {
    loading.value = true
    const payloadDate = toLocalDateTimeStartOfDay(weekStart.value)
    const result = await svc.payAllByWeek(payloadDate)
    paidCount.value = Array.isArray(result) ? result.length : 0
    toast.success('Pagos marcados como pagados')
  } catch (e: any) {
    const msg = e?.data?.message || 'No se pudieron realizar los pagos'
