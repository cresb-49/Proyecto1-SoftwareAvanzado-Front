    ...
    <div class="mb-4 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-brand-700">
          Listado con hotel, restaurante y rol del usuario.
        </p>
      </div>
      <div class="flex items-center gap-2">
        <Button size="sm" variant="primary" to="/pagos/crear"
          >Crear los pagos</Button
        >
        <Button size="sm" variant="secondary" to="/pagos/realizar">
          Relizar pagos
        </Button>
      </div>
    </div>
    ...
    >
      ...
      <template #cell-acciones="{ row }">
        <div class="flex items-center justify-end gap-2">
          <Button size="sm" variant="primary" :to="`/pagos/empleado/${row.id}`"
            >Pagos</Button
          >
          <Button
            size="sm"
            variant="secondary"
            :to="`/pagos/empleado/crear/${row.id}`"
            >Crear Pago</Button
          >
        </div>
      </template>
...
const {
  ...
  error,
  refresh,
} = await useAsyncData("employees:list", () => employeeSvc.getAll(), {
  server: true,
});
...
const hotelIds = computed(() =>
  Array.from(new Set(employees.value.map((e) => e.hotelId).filter(Boolean)))
);
const restaurantIds = computed(() =>
  Array.from(
    new Set(employees.value.map((e) => e.restaurantId).filter(Boolean))
  )
);
const userIds = computed(() =>
  Array.from(new Set(employees.value.map((e) => e.userId).filter(Boolean)))
);
  ...
  try {
    const [hotels, restaurants, users] = await Promise.all([
      hotelIds.value.length
        ? Promise.all(
            hotelIds.value.map((id) =>
              hotelSvc.getById(String(id)).catch(() => null)
            )
          )
        : Promise.resolve([]),
      restaurantIds.value.length
        ? Promise.all(
            restaurantIds.value.map((id) =>
              restaurantSvc.getById(String(id)).catch(() => null)
            )
          )
        : Promise.resolve([]),
      userIds.value.length
        ? Promise.all(
            userIds.value.map((id) =>
              userSvc.getById(String(id)).catch(() => null)
            )
          )
        : Promise.resolve([]),
    ]);
    hotelMap.value = Object.fromEntries(
      (hotels as any[]).filter(Boolean).map((h: any) => [h.id, h])
    );
    restaurantMap.value = Object.fromEntries(
      (restaurants as any[]).filter(Boolean).map((r: any) => [r.id, r])
    );
    userMap.value = Object.fromEntries(
      (users as any[]).filter(Boolean).map((u: any) => [u.id, u])
    );
  } finally {
...
async function onToggleState(row: Row, next: boolean) {
  try {
    ...
    const user: any =
      userMap.value[employees.value.find((e) => e.id === row.id)?.userId || ""];
    if (!user?.id) {
